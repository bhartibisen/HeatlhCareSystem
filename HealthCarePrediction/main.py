...
# Load datasets
sys_des = pd.read_csv(r"datasets\description.csv")
precautions = pd.read_csv(r"datasets\precautions.csv")
workout = pd.read_csv(r"datasets\workout.csv")
...
def helper(dis):
    ...
    desc = " ".join(desc.values)  # convert to string

    prec = precautions[precautions['Disease'] == dis][['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']]
    prec = prec.values.flatten().tolist()

    medi = medications[medications['Disease'] == dis]['Medication']
    medi = medi.values.flatten().tolist()

    diet = diets[diets['Disease'] == dis]['Diet']
    diet = diet.values.flatten().tolist()

    wrkout = workout[workout['disease'] == dis]['workout']
    wrkout = wrkout.values.flatten().tolist()

    return desc, prec, medi, diet, wrkout
...
def predict():
    if request.method == 'POST':
        ...
        desc, prec, medi, diet, wrkout = helper(predicted_disease)

        # Flatten all if needed
        def flatten(data):
            if isinstance(data, list) and all(isinstance(i, list) for i in data):
                return [item for sublist in data for item in sublist]
            return list(data) if hasattr(data, 'values') else data

        prec = flatten(prec)
        medi = flatten(medi)
        diet = flatten(diet)
        wrkout = flatten(wrkout)
